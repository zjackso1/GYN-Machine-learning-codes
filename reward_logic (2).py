...
def compute_reward(
    ...
    for organ, dose in oar_doses.items():
        ...
        if use_baseline and organ in baseline_oar_doses:
            ...
            if delta > 0.0:
                ...
                oar_sparing += weight * ((-delta) / denom)

        else:
            if dose > limit:
                over = float(dose - limit)
                denom = max(limit, 1e-6)
                oar_penalty += weight * ((over / denom) ** 2)
    ...
    if stop:
        if in_d90_band:
            ...
            if hrctv_d90 < d90_low:
                band_violation = float(d90_low - hrctv_d90)
            else:
                band_violation = float(hrctv_d90 - d90_high)
